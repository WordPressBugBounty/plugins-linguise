...
jQuery(document).ready(($) => {
    ...
    function updateConfigDemoScript(newConfig) {
        ...
        instance.is_joomla = false;
        instance.is_wp = false;
        if (typeof instance.injectCustomCSS === 'function') {
            instance.injectCustomCSS();
        }
        instance.addFlag(instance.language_settings);
    }
    ...
    $('[data-linguise-action="clear-cache"]').on('click', (ev) => {
        ...
        $.ajax({
            ...
            error: (xhr, status, error) => {
                ...
                let baseElement = '<div class="linguise-notification-popup"><span class="material-icons fail">close</span> ';
                if (responseData && responseData.message) {
                    baseElement += responseData.message;
                } else {
                    baseElement += 'Failed to clear cache!';
                }
                baseElement += '</div>';
                const elemMade = $(baseElement);
                $('body').append(elemMade);
                linguiseDelayHideNotification(elemMade);
            },
        });
    ...
    $('[data-linguise-action="clear-debug"]').on('click', (ev) => {
        ...
        $.ajax({
            ...
            success: (data) => {
                if (!data.error) {
                    const elemMade = $('<div class="linguise-notification-popup"><span class="material-icons">done</span> ' + data.message + '</div>');
                    $('body').append(elemMade);
                    linguiseDelayHideNotification(elemMade);
                } else {
                    console.error('Clear debug:', data.message);
            ...
            error: (xhr, status, error) => {
                ...
                if (responseData && responseData.message) {
                    ...
                    baseElement += 'Failed to clear debug file!';
                }
                baseElement += '</div>';
                const elemMade = $(baseElement);
                $('body').append(elemMade);
    ...
    // Logout button
    document.querySelectorAll('[data-linguise-action="logout"]').forEach((logoutBtn) => {
        logoutBtn.addEventListener('click', (ev) => {
            ev.preventDefault();

            const href = new URL(logoutBtn.getAttribute('data-href'));
            href.searchParams.set('linguise_action', 'logout');
            ...
            $.ajax({
                ...
                success: (data) => {
                    if (!data.error) {
                        ...
                        const elemMade = $('<div class="linguise-notification-popup"><span class="material-icons fail">close</span> Failed to logout!</div>');
                        $('body').append(elemMade);
                        linguiseDelayHideNotification(elemMade);
                    }
                },
